...
lemma mod_converter (ha: a % n = b) : a ≡ b [MOD n] := by
  rw [← ha]
  exact (Nat.mod_modEq a n).symm

lemma lemma1 : ((a % n) ^ b) % n = (a ^ b) % n := (Nat.pow_mod _ _ _).symm

lemma lemma2 : (a * b) % n = ((a % n) * (b % n)) % n := Nat.mul_mod _ _ _

variable (p q n : ℕ)

lemma lemma3 (h : p ≡ 1 [MOD n]) (hn : 0 < n) (hp : 1 ≤ p) : ∃ k : ℕ, p = 1 + k * n := by
  -- convert modulo to |
  have h_symm : 1 ≡ p [MOD n] := h.symm
...
theorem lemma4 (m k p q : ℕ) -- Chinese Remainder Theorem
  ...
  have h0 : 1 < p * q := by
    ...
    have hp0 : 0 < p := prime_p.pos
    have hq0 : 0 < q := prime_q.pos
    -- Since p and q are prime, they are greater than 1
    have hp1 : 1 < p := prime_p.one_lt
  ...
  have h1 : m ^ (p - 1) % p = 1 := by
    ...
    rw [← this.symm]
    exact Nat.mod_eq_of_lt prime_p.one_lt
  ...
  have h2 : m ^ (q - 1) % q = 1 := by
    ...
    rw [← this.symm]
    exact Nat.mod_eq_of_lt prime_q.one_lt

  -- -- Step 3: Handle powers of (p-1)(q-1) mod p and q
  have h3 : m ^ ((p-1) * (q-1)) % p = 1 := by
    -- Nat.pow_mod:  a ^ b % n = (a % n) ^ b % n
    rw [Nat.pow_mul, Nat.pow_mod, h1]
    simp only [one_pow]
    exact Nat.mod_eq_of_lt prime_p.one_lt

  have h4 : m ^ ((p-1) * (q-1)) % q = 1 := by
    -- change to m ^ ((q-1) * (p-1)) % q
    rw [Nat.mul_comm, Nat.pow_mul, Nat.pow_mod, h2]
    simp only [one_pow]
    exact Nat.mod_eq_of_lt prime_q.one_lt

  -- Step 4: Extend to k multiples
  have h5 : m ^ (k * (p-1) * (q-1)) % p = 1 := by
    rw [Nat.mul_right_comm, Nat.mul_assoc, Nat.mul_comm, Nat.pow_mul, Nat.mul_comm, Nat.pow_mod, h3]
    simp only [one_pow]
    exact Nat.mod_eq_of_lt prime_p.one_lt

  have h5':  m ^ (k * (p-1) * (q-1)) ≡ 1 [MOD p] := mod_converter _ _ _ h5

  have h6 : m ^ (k * (p-1) * (q-1)) % q = 1 := by
    rw [Nat.mul_right_comm, Nat.mul_assoc, Nat.mul_comm, Nat.pow_mul, Nat.pow_mod, Nat.mul_comm, h4,
      one_pow]
    exact Nat.mod_eq_of_lt (Nat.Prime.one_lt prime_q)

  have h6':  m ^ (k * (p-1) * (q-1)) ≡ 1 [MOD q] := mod_converter _ _ _ h6
...
theorem rsa_correctness (prime_p: Nat.Prime p) (prime_q: Nat.Prime q)
  ...
  · simp only [pow_one, mul_one, dvd_refl, Nat.mod_mod_of_dvd]

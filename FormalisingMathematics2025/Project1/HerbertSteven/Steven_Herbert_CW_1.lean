...
lemma Prime_Factors (a p : ℤ) (h1 : Prime p) (h2 : a ∣ p) : a = 1 ∨ a = p := by
  sorry
  done

/- Lemma : if `p` is prime, and an integer `a` is such that `p ∣ a` then the greatest common divisor
of `p` and `a` is `p` -/
lemma Simple_GCD_lemma_1 {p a : ℤ} (h1 : p ∣ a) (h2: Prime p) : gcd p a = p := by
...
lemma Simple_GCD_lemma_2 {p a : ℤ} (h1 : ¬ (p ∣ a)) (h2 : Prime p) : gcd p a = 1 := by
  ...
  have h5 : gcd p a = 1 ∨ gcd p a = p := Prime_Factors (gcd p a) p h2 h3
...
lemma Primes_are_Coprime {p1 p2 : ℤ} (h1 : Prime p1)
  ...
  have h6 : gcd p1 p2 = 1 ∨ gcd p1 p2 = p1 := Prime_Factors (gcd p1 p2) p1 h1 h4
  have h7 : gcd p1 p2 = 1 ∨ gcd p1 p2 = p2 := Prime_Factors (gcd p1 p2) p2 h2 h5
...
lemma Euclids_lemma {p a b : ℤ} (h1 : p ∣ a * b) (h2 : Prime p )
    : ¬ p ∣ a  → p ∣ b := by
  intro h3
  have h4 : gcd p a = 1 := Simple_GCD_lemma_2 h3 h2
  ...
  have h7 : ∀ x1 : ℤ , p ∣ p * (x1 * b) := by
    intro x1
    exact Dvd.dvd.mul_right h6 (x1 * b)
  have h8 : ∀ y1 : ℤ , p ∣ a * b * y1 := by
    intro y1
    exact Dvd.dvd.mul_right h1 y1
...
theorem Euclids_lemma_alternative {p a b : ℤ} (h1 : p ∣ a * b) (h2 : Prime p ) : p ∣ a ∨ p ∣ b := by
  have h3 : ¬ p ∣ a → p ∣ b := Euclids_lemma h1 h2
  by_contra
  aesop
  done
...
theorem Semi_Primes_Unique_Factorisation {q1 q2 p c : ℤ} (h1 : Prime p)
    (h2 : Prime q1)(h3 : Prime q2)(h4 : p ≠ q1)(h5 : p ≠ q2) : p * c ≠ q1 * q2 := by
  intro h6

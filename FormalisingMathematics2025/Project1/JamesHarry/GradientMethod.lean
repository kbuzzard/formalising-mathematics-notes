...
lemma directional_derivative_eq_inner_product:
  directional_derivative f x d = ⟪gradient f x, d⟫ := by
  rw [directional_derivative, gradient]
  exact Eq.symm InnerProductSpace.toDual_symm_apply


lemma directional_derivative_lower_bound (hf : DifferentiableAt ℝ f x) (hd: ‖d‖=1) :
  directional_derivative f x d ≥ - ‖gradient f x‖ := by
  rw [directional_derivative_eq_inner_product]

  have h1 : |⟪(gradient f x), d⟫| ≤ ‖(gradient f x)‖ * ‖d‖ := by
    exact abs_cauchy_schwarz (gradient f x) d
...
theorem minus_the_gradient_at_x_is_the_direction_of_steepest_local_descent
  (hf : DifferentiableAt ℝ f x) (hx : gradient f x ≠ 0) (hd : ‖d‖ = 1) :
  directional_derivative f x d ≥
  directional_derivative f x ((-1/‖gradient f x‖) • (gradient f x)) := by
  rw [directional_derivative, directional_derivative_eq_inner_product]
  ...
  have h1 : ⟪gradient f x, (-1 / ‖gradient f x‖) • gradient f x⟫  = -‖gradient f x‖ := by
    ...
    field_simp [norm_ne_zero_iff.2]
    exact pow_two ‖gradient f x‖

  rw [h1]
  rw [← directional_derivative]
  exact directional_derivative_lower_bound hf hd

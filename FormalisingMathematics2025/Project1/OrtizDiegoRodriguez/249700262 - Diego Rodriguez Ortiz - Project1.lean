...
def MySubgroup [MyGroup G] (S : Set G) : Prop :=
  (∀ a b, a ∈ S ∧ b ∈ S → a * b ∈ S) ∧ 1 ∈ S ∧ ∀ a ∈ S, a⁻¹ ∈ S
-- BM: at the top of page 5 of your report you talk a bit about this choice of defn and how it works
-- in fact subgroups used to be defined like this in Lean (maybe about 5/6 years ago), but now the
-- approach is the way that's in the course repo in section 7.
-- the distinction happens when you want to do homomorphisms or unions on subgroups: if you work
-- with them just as sets then this approach is awkward, but the mathlib one works nicer.

-- Subgroup definition
lemma subgroup_def [MyGroup G] (S: Set G) :
    MySubgroup S ↔ (∀ a b, a ∈ S ∧ b ∈ S → a * b ∈ S) ∧ 1 ∈ S ∧ ∀ a ∈ S, a⁻¹ ∈ S := by
  rfl
  -- BM: it's usually good to change p ∧ q → r to p → q → r; these are logically equivalent but
  -- the latter is slightly easier to use most of the time (you don't need to keep doing constructor
  -- or cases)
  -- Note I also changed the formatting of your def and lemma: the infoview is telling me that
...
lemma trivial_one_mem (G : Type) [MyGroup G] (g : G) : g ∈ trivialSubgroup G ↔ g = 1 :=
...
lemma trivial_is_subgroup (G : Type) [MyGroup G] : MySubgroup (trivialSubgroup G) := by
  ...
  · -- Proof: a ∈ trivialSubgroup ∧ b ∈ trivialSubgroup → a * b ∈ trivialSubgroup
    intro a b hab
    -- BM: This might not be better but I wanted to show it to you anyway
    -- hover your mouse over the simp_all to see its description
    simp_all [trivial_one_mem, one_mul]
...
lemma homomorphism_def [MyGroup G] [MyGroup H] (φ: G → H) :
    MyHomomorphism φ ↔ ∀ a b : G, φ (a * b) = φ a * φ b := by
  rfl
...
(2) Second result:
    Let `φ: G → H` be an homomorphism then `∀ g ∈ G , φ g⁻¹ = (φ g)⁻¹`.
-/
theorem homo_img_inv [MyGroup G] [MyGroup H] (φ : G → H) (h_homo : MyHomomorphism φ) (g : G) :
    (φ g)⁻¹ = φ g⁻¹ := by
...
(3) Third result:
    Let `φ : G → H` be an homomorphism then then image of `φ` is a subgroup of `H`.
-/
theorem img_subgroup [MyGroup G][MyGroup H] (φ : G → H) (h_homo : MyHomomorphism φ) :
    MySubgroup (image φ) := by
  -- Definition of subgroup
  rw [subgroup_def]
...
-/
lemma ker_mem_iff_img_one [MyGroup G] [MyGroup H] (φ : G → H) (g : G) :
    g ∈ kernel φ ↔ φ g = 1 := by rfl

/-!
(4) Forth result:
    Let `φ : G → H` be an homomorphism then then kernel of `φ` is a subgroup of `G`.
-/
theorem ker_subgroup [MyGroup G] [MyGroup H] (φ : G → H) (h_homo : MyHomomorphism φ) :
    MySubgroup (kernel φ) := by
  rw [subgroup_def]
  -- Divide the proof in proving the three conditions for a set to being a subgroup
  constructor
...
(5) Fifth result:
    Let `φ : G → H` be an homomorphism then `kernel φ = {1}` ↔ `φ` injective`
-/
theorem injective_iff_ker_one [MyGroup G] [MyGroup H] (φ : G → H) (h_homo : MyHomomorphism φ) :
    Function.Injective φ ↔ kernel φ = trivialSubgroup G := by
  -- Divide the ↔ into → and ←
  constructor
  ...
  · intro h_ker
    ...
    rw [inv_mul_self]
    apply h_only_1 (b⁻¹ * a)
    -- b⁻¹ * a ∈ kernel φ ← φ (b⁻¹ * a) = 1 ← φ b⁻¹ * φ a = (φ b)⁻¹ * φ b
    -- ← φ b⁻¹ * φ b = φ b ⁻¹ * φ b
...
(6) Sixth result:
    Let `φ : G → H` and `ψ: H → K` be homomorphisms then `ψ ∘ φ: G → K` is an homorphism.
-/
theorem homo_composition [MyGroup G] [MyGroup H] [MyGroup K] (φ : G → H) (ψ : H → K)
    (h_phi : MyHomomorphism φ) (h_psi : MyHomomorphism ψ) : MyHomomorphism (ψ ∘ φ) := by
